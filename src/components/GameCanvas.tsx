...
import { MAP_W, MAP_H, COAST_TOP, COAST_BOTTOM, SANDBANKS, HEAL_ZONES } from '../game/map';
...
export function GameCanvas({ state }: Props) {
  ...
  useEffect(() => {
    ...
    function render() {
      ...
      for (let y = COAST_TOP + 20; y < COAST_BOTTOM; y += 40) {
        const offset = Math.sin(whalePhase * 0.6 + y * 0.01) * 10;
        ctx.fillRect(0, y + offset, MAP_W, 2);
      }
      ctx.globalAlpha = 1;

      // (coasts are now drawn via the organic sandbank polygons below)

      // Heal zones (bagger-rinnen)
      ...
      for (const sb of SANDBANKS) {
        ...
        // subtle grass tufts on larger banks
        ...
        if (sb.name) {
          ...
          ctx.textAlign = 'center';
          ctx.fillText(sb.name, sb.x, sb.y + 4);
          ctx.restore();
        }
      }
...
function HUD({ state }: { state: GameState }) {
  ...
  return (
    <>
      ...
      {state.whale.bargeTimer > 0 && state.whale.hp > 0 && (
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 bg-emerald-500/90 text-white font-bold px-6 py-2 rounded-full shadow-lg">
          Timmy betritt die Barge... {Math.max(0, 3 - state.whale.bargeTimer).toFixed(1)}s
        </div>
      )}
    </>
  );
