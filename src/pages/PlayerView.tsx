...
import { usePlayer, type ConnectionStatus } from '../game/usePlayer';
...
// This is now defined at the top as PlayerViewProps


function DisconnectedOverlay({ status }: { status: ConnectionStatus }) {
  if (status === 'connected') return null;
  return (
    <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-sm z-50 flex flex-col items-center justify-center text-center p-4">
      <div className="mb-8">
        <Wifi className="w-12 h-12 text-blue-500 mx-auto mb-4 animate-pulse" />
        <div className="font-semibold text-xl mb-2 text-white">
          {status === 'disconnected' ? 'Verbindung unterbrochen' : 'Verbinde...'}
        </div>
        <div className="text-slate-400 text-sm">
          {status === 'disconnected' ? 'Versuche, die Verbindung wiederherzustellen...' : 'Bitte einen Moment Geduld.'}
        </div>
      </div>
    </div>
  );
...
export function PlayerView({ code, name, playerId, onLeave }: PlayerViewProps) {
  const { state, role, assignments, ping, setInput, pressConference, vote, ready, connectionStatus } = usePlayer(
    code,
    playerId,
    name
  );
      ...
      <div className="min-h-[100dvh] flex flex-col items-center justify-center text-center p-4">
        ...
        <div className="mb-8">
          ...
          <div className="text-slate-500 text-sm">
            Bitte warten, oder kehre zur Startseite zurück, falls der Raum nicht mehr existiert.
          </div>
        </div>
        <button
          onClick={onLeave}
  ...
  return (
    <>
      ...
      {(state.phase === 'starting' || state.phase === 'ready') && (
              ...
              <div className={`transition-opacity duration-300 ${isRevealingRole ? 'opacity-100' : 'opacity-0'}`}>
                {isImposter ? (
                  ...
                  <>
                    ...
                    <div className="text-sm text-green-700">Lotst Timmy sicher in die Barge!</div>
                  </>
                )}
              </div>
              <div
          ...
          >
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                <div
                  className="flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-black/20 text-[10px] text-white font-mono"
                  title="Ping"
                >
                  <Wifi
                    className={`w-2.5 h-2.5 ${
                      ping === 0 ? 'opacity-50' : ping < 150 ? 'text-green-400' : ping < 400 ? 'text-yellow-400' : 'text-red-400'
                    }`}
                  />
